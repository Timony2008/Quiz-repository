...
export default function AddQuizForm({
  ...
  return (
    <form onSubmit={onSubmit} style={{ marginBottom: 20, padding: 14, border: '1px solid #ddd', borderRadius: 8 }}>
      <textarea
        placeholder="题目 *" value={question} onChange={e => onQuestionChange(e.target.value)} rows={3}
      ...
      <textarea
        placeholder="答案 *" value={answer} onChange={e => onAnswerChange(e.target.value)} rows={3}
        style={{ width: '100%', marginBottom: 4, padding: '6px 10px', boxSizing: 'border-box', resize: 'vertical', fontFamily: 'monospace' }}
      />
      {answer && (
        <div style={{ fontSize: 13, color: '#555', padding: '4px 8px', background: '#fafafa', borderRadius: 4, marginBottom: 8 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
        <input
          type="number" placeholder="难度（1 ~ 7，如 4.5）"
          min={1} max={7} step={0.1}
          value={difficulty} onChange={e => onDifficultyChange(e.target.value)}
